...
This research question is a regression problem; we are trying to predict salary, a continuous outcome variable. The target variable is `adj_salary`, found in the `nba_seasons` dataset under the `data` folder. This variable measures a player’s yearly salary adjusted to 2023 prices using the [Consumer Price Index for All Urban Consumers from the US Bureau of Labor Statistics.](https://www.bls.gov/data/inflation_calculator.htm). 
