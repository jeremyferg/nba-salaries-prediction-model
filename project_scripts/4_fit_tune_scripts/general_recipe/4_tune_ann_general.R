############################################################
############################################################
#######                                             ########
######      TUNE NEURAL NETWORKS MODEL NONLINEAR     #######
#######                                             ########
############################################################
############################################################


#####################
##### Libraries #####
#####################

library(tidyverse)
library(tidymodels)
library(here)
library(parallel)

tidymodels_prefer()

# parallel processing
doParallel::registerDoParallel(detectCores(logical = TRUE))

#####################
##### Data Sets #####
#####################

load(here('recipes/nba_recipe_general.rda'))
load(here("data/splits_folds/nba_seasons_folds_base.rda"))
load(here('results/keep_wflow_rsample.rda'))

################################################################################
################################################################################
################################################################################

# elastic net
ann_spec <-
  mlp(
    penalty = tune(),
    hidden_units = tune(),
    epoch = tune()
  ) |> 
  set_engine('nnet') |> 
  set_mode('regression') 

# defining workflow
ann_general_wflow <-
  workflow() |> 
  add_model(ann_spec) |> 
  add_recipe(nba_recipe_general)

# hyperparameter tuning values ----
extract_parameter_set_dials(ann_spec)

# change hyperparamter ranges
ann_general_params <- extract_parameter_set_dials(ann_spec) |> 
  # N:= maximum number of random predictor columns we want to try 
  # should be less than the number of available columns
  update(hidden_units = mixture(range = c(0, 5)),
         penalty = penalty(range = c(-10, 0),),
         epochs = epochs(range = c(100, 750)))

# build tuning grid
ann_grid <- grid_regular(ann_general_params, levels = 5)

# resampling
set.seed(56709)
ann_general_tuned <- 
  ann_general_wflow |> 
  tune_grid(
    nba_seasons_folds_base, 
    grid = ann_grid,
    control = keep_wflow_rsample
  )

# write out results (fitted/trained workflows) ----

save(ann_general_tuned, file = here('results/general_recipe/ann_general_tuned.rda'))
